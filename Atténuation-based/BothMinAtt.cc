...
void RoutGeo2::handleUpperMsg(cMessage* m) {
...
void RoutGeo2::handleLowerMsg(cMessage* m)

{
    ...
    else
    {
        ...
        {
            if(msg->getDestAddr()==myNetwAddr || msg->getDestAddr2()==myNetwAddr)
            {
                if (msg->getTtl()>1)
                {
...
bool RoutGeo2::notBroadcasted(NetwPkt* msg) {
...
void RoutGeo2::forwardControl(NetwPkt *msg)
{
...
void RoutGeo2::recuControl(NetwPkt *msg)
{
    int x=msg->getInitialsrcAddr();
    switch (x)
    {
...
void RoutGeo2::multiSend(NetwPkt* msg)
{
    //BGW
    NetwPkt* msgCopy;
    int nbT=0;
...
NetwPkt* RoutGeo2::encapsMsg(cPacket *appPkt) {
...
void RoutGeo2::handleSelfMsg(cMessage *m)
{
    //NetwPkt *msg = static_cast<NetwPkt *> (m);
    int reREQ=0;
    ...
    if(BestAtt[0][0]!=-1)
    {
        while(!netQueue.empty())
        {
            for (int i=0;i<2;i++)
            {
                if(BestAtt[0][i]!=-1 )
                {
                    EV<<"me "<<myNetwAddr<<"  NextHOP égal à "<<BestAtt[0][i]<<endl;
                    NetwPkt *dMsg = static_cast<NetwPkt *>(netQueue.front()->dup());
                    dMsg->setSrcAddr(myNetwAddr);
                    dMsg->setDestAddr((int)BestAtt[0][i]);
                    if(optiEnable==0)
                    {
    ...
    if(BestAtt[0][0]!=-1)
    {
        ...
        if(BestAtt[0][1]!=-1)
        {
            dMsg->setDestAddr2((int)BestAtt[0][1]);
        }
        else
            dMsg->setDestAddr2(-1);
        if(optiEnable==0)
