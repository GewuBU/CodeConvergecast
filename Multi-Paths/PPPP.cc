...
#include "CnSmp1Pkt_m.h"
#include "SimTracer.h"
#include "LogNormalShadowing.h"

Define_Module(CnSmp1);

void CnSmp1::initialize(int stage)
{
...
void CnSmp1::handleLowerMsg(cMessage* msg)
{
    EV<<"LLLLLLLLLLOOOOOOOOOWWWWWWWWWWWWEEEEEEEEEEEERRRRRRRRRRRRRRRRRRR : "<<myNetwAddr<< endl;
    CnSmp1Pkt*           netwMsg        = check_and_cast<CnSmp1Pkt*>(msg);
    ...
    if (finalDestAddr!=myNetwAddr && finalDestAddr1!=myNetwAddr) {
        ...
        if (netwMsg->getKind()==DATA && myNetwAddr!=1){
             msgCopy = check_and_cast<CnSmp1Pkt*>(netwMsg->dup());
             ...
             while (i<=3){
              ...
              if (rep1==1){
               ...
               netwMsg->setFinalDestAddr1(2540);
               EV<<"j'ai choisis le : "<<netwMsg->getFinalDestAddr()<<endl;
               rep1=0;
...
void CnSmp1::handleLowerControl(cMessage *msg)
{
    delete msg;
}

void CnSmp1::handleUpperMsg(cMessage* msg)
{
    //BGW
    const cObject* pCtrlInfo = NULL;
    ...
    for (i=0;i<=5; i++)
        EV<<tab[i]<<endl;
    i=0;
    CnSmp1Pkt*    pkt   = new CnSmp1Pkt(msg->getName(), DATA);
    rnd=rand() % 2;
    if (rnd==0)
...
bool CnSmp1::notBroadcasted(CnSmp1Pkt* msg) {
...
void CnSmp1::forwardControl(CnSmp1Pkt *msg)
{
...
void CnSmp1::recuControl(CnSmp1Pkt *msg)
{
    int x=msg->getInitialSrcAddr();
    switch (x)
    {
...
void CnSmp1::multiSend(CnSmp1Pkt* msg)
{
    //BGW
    CnSmp1Pkt* msgCopy;
    int nbT=0;
...
cMessage* CnSmp1::decapsMsg(CnSmp1Pkt *msg)
{
